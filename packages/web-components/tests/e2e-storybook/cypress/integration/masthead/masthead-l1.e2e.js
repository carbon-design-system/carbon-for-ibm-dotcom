...
describe('cds-masthead | with L1 (desktop)', () => {
  ...
  it('should render platform below the IBM logo', () => {
    cy.get('cds-masthead-l1-name').then($platform => {
      cy.get('cds-masthead-logo').then($logo => {
  ...
  it('should load l1 menu item with selected state', () => {
    cy.get('cds-top-nav-l1 > *:nth-child(1)').then($menuItem => {
  ...
  it('should render 5 menu items', () => {
    cy.get('cds-top-nav-l1 > * ').should('have.length', 5);
  });

  it('should load the l1 - first nav item', () => {
    cy.get('cds-top-nav-l1 > *:nth-child(1)')
      .shadow()
      .find('a')
      .then($link => {
        const url = $link.prop('href');
  ...
  it('should load the l1 - second nav item', () => {
    cy.get('cds-top-nav-l1 > *:nth-child(2)')
      .shadow()
      .find('a')
      .then($link => {
        const url = $link.prop('href');
  ...
  it('should load and have url for third l1 item', () => {
    cy.get('cds-top-nav-l1 > *:nth-child(3)')
      .shadow()
      .find('a')
      .then($link => {
        const url = $link.prop('href');
  ...
  it('should load the l1 - fourth nav item', () => {
    cy.get('cds-top-nav-l1 > *:nth-child(4)')
      .click()
      .then($menuItem => {
  ...
  it('should load and have url for fifth l1 item', () => {
    cy.get('cds-top-nav-l1 > *:nth-child(5)')
      .click()
      .then($menuItem => {
        expect($menuItem).to.have.attr('expanded');
      });
