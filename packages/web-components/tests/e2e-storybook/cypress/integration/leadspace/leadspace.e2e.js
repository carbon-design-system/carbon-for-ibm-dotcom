 ...
 * @private
 */
const _pathShortWithImage =
  '/iframe.html?id=components-lead-space--short-with-image';
...
describe('c4d-leadspace | tall', () => {
  ...
  it('should load title and copy - both left aligned', () => {
    ...
    cy.get('c4d-leadspace-heading').then(($title) => {
      expect($title[0].getBoundingClientRect().left).to.equal(16);
    });

    cy.get('[data-autoid="c4d--leadspace__desc"]').then(($desc) => {
  ...
  it.skip('should render 3 buttons with different icons (arrow right, left, and PDF)', () => {
    ...
    cy.get('c4d-button-group-item:nth-child(1) svg path').then(($icon) => {
    ...
    cy.get('c4d-button-group-item:nth-child(2) svg path').then(($icon) => {
      expect($icon).to.have.attr(
        'd',
    ...
    cy.get('c4d-button-group-item:nth-child(3) svg path').then(($icon) => {
      expect($icon).to.have.attr(
        'd',
  ...
  it('should load more than 2 buttons when customized and should all have links', () => {
    ...
    cy.get('c4d-button-group-item').should('have.length', 3);
  ...
  it('should load without a background image', () => {
    ...
    cy.get('c4d-background-media').should('not.exist');
  });
...
describe('c4d-leadspace | tall with image', () => {
  it('should check a11y', () => {
    cy.visit(`/${_pathTall}`);
    cy.injectAxe();
    cy.checkAxeA11y();
...
describe('c4d-leadspace | centered', () => {
  it('should check a11y', () => {
    cy.visit(`/${_pathTall}`);
    cy.injectAxe();
    cy.checkAxeA11y();
  ...
  it.skip('should load buttons centered aligned', () => {
    ...
    cy.get('c4d-button-group').then(($button) => {
  ...
  it('should render 3 buttons with different icons (arrow right, left, and PDF)', () => {
    cy.visit(
      `/${_pathCentered}&knob-Number%20of%20buttons=3&knob-Icon%201=ArrowRight20&knob-Button%201=Button%201&knob-Icon%202=ArrowDown20&knob-Icon%203=Pdf20`
    );
    cy.viewport(1280, 780);
...
describe('c4d-leadspace | centered with image', () => {
  it('should check a11y', () => {
    cy.visit(`/${_pathTall}`);
    cy.injectAxe();
    cy.checkAxeA11y();
...
describe('c4d-leadspace | short with image', () => {
  it('should check a11y', () => {
    cy.visit(`/${_pathTall}`);
    cy.injectAxe();
    cy.checkAxeA11y();
...
describe('c4d-leadspace | medium with image', () => {
  it('should check a11y', () => {
    cy.visit(`/${_pathTall}`);
    cy.injectAxe();
    cy.checkAxeA11y();
...
describe('c4d-leadspace | super with image', () => {
  it('should check a11y', () => {
    cy.visit(`/${_pathTall}`);
    cy.injectAxe();
    cy.checkAxeA11y();
  ...
  it.skip('should load with background image', () => {
    ...
    cy.get('c4d-background-media').then(($ele) => {
