...
const _paths = {
  default:
    'iframe.html?id=components-cta--default&knob-CTA%20style%20(cta-style)=card-link',
  types:
...
const _tests = {
  ...
  checkHoverStyles: () => {
    cy.get('c4d-card-link-cta').then(([card]) => {
      const sheets = card.shadowRoot.adoptedStylesheets;

      if (sheets) {
        const hover = getCssPropertyForRule(
          '.cds--card:hover, :host(c4d-card:hover), :host(c4d-card-group-item:hover), :host(c4d-card-group-item) .cds--card:hover, :host(c4d-card-cta:hover), :host(c4d-card-in-card:hover), :host(c4d-content-group-cards-item:hover), :host(c4d-content-group-cards-item) .cds--card:hover',
          'background-color',
          sheets
        );
