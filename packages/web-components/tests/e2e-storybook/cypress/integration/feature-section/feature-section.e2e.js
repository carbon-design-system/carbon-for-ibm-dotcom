...
describe('dds-feature-section (mobile)', () => {
  ...
  it('should have content on the top and image on the bottom (media align: right)', () => {
    ...
    // content takes the top half
    cy.get('dds-feature-section')
      .shadow()
      .find('.bx--feature-section__body')
      .then($content => {
        expect($content[0].getBoundingClientRect().top).to.equal(16);
        expect($content[0].getBoundingClientRect().bottom).to.equal(394);
      });

    // image takes the bottom half
    cy.get('dds-image').then($image => {
      expect($image[0].getBoundingClientRect().top).to.equal(394);
      expect($image[0].getBoundingClientRect().bottom).to.equal(714);
    });
