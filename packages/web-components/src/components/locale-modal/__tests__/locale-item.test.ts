...
const template = (props?) => {
  ...
  return html`
    <dds-locale-item country="${ifNonNull(country)}" language="${ifNonNull(language)}"></dds-locale-item>
  `;
};
...
describe('dds-locale-item', function() {
  describe('Misc attributes', function() {
    it('should render with various attributes', async function() {
      render(template({ country: 'country-foo', language: 'language-foo' }), document.body);
