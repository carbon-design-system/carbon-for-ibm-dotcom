...
describe('dds-table-of-contents', () => {
  describe('With wide screen', () => {
    describe('Without heading content', function () {
      beforeEach(async () => {
        ...
        await page.goto(
          `http://localhost:${process.env.PORT}/iframe.html?id=components-table-of-contents--default`
        );
      });

      it('should hide the heading container unless it has its content', async () => {
        await expect(page).toHaveSelector(
          'dds-table-of-contents .cds--tableofcontents__desktop__children',
          { state: 'hidden' }
        );
      });
    });
    ...
    describe('With heading content', () => {
      beforeEach(async () => {
        await page.setViewportSize({ width: 1280, height: 720 });
        await page.goto(
          `http://localhost:${process.env.PORT}/iframe.html?id=components-table-of-contents--with-heading-content`
        );
      });
  ...
  describe('With narrow screen', () => {
    describe('Without heading content', function () {
      ...
      it('should hide the heading container unless it has its content', async () => {
        await expect(page).toHaveSelector(
          'dds-table-of-contents .cds--tableofcontents__children__mobile',
          { state: 'hidden' }
        );
      });
    });
