...
const generateCopySections = (n: Number) =>
  [...Array(n)].map(() => <p>{LOREM}</p>);

const content = ({ items }: { items: Array }) => (
  <div className="cds--tableofcontents__contents cds-ce-demo--table-of-contents">
    {items?.map(({ heading, copy }, i: Number) => (
      <>
        ...
        {copy
          ? copy.split('\n').map((section: String) => <p>{section}</p>)
