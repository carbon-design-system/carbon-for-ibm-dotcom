...
* The Card component will need to be wrapped within a `dds-video-cta-container` components for videos to play.
* If left empty, the `dds-card-footer` can display the video duration. If not, it will be overridden with custom text.

General note:
* Custom icons can replace the built-in ones within `dds-card-footer`, they only need to be assigned to the `icon` slot.
