import { Props, Description } from '@storybook/addon-docs/blocks';
import markdownContents from '../../../../docs/markdown-contents.md';
import contributing from '../../../../../../docs/contributing-license.md';
import { cdnJs, cdnCss } from '../../../globals/internal/storybook-cdn';

# `<c4d-cta-block>`

> 💡 Check our
> [Stackblitz](https://stackblitz.com/github/carbon-design-system/carbon-for-ibm-dotcom/tree/main/packages/web-components/examples/stackblitz/components/cta-block)
> example implementation.
> [![Edit @carbon/ibmdotcom-web-components](https://codesandbox.io/static/img/play-codesandbox.svg)](https://stackblitz.com/github/carbon-design-system/carbon-for-ibm-dotcom/tree/main/packages/web-components/examples/codesandbox/components/cta-block)

## Getting started

Here's a quick example to get you started.

### JS (via import)

```javascript
import '@carbon/ibmdotcom-web-components/es/components/cta-block/index.js';

// with link list
import '@carbon/ibmdotcom-web-components/es/components/link-list/index.js';
```

<Description markdown={`${cdnJs({ components: ['cta-block'] })}`} />

<Description markdown={`${cdnCss()}`} />

### HTML

```html
<c4d-cta-block no-border>
  <c4d-content-block-heading
    >C4D Content Block Heading</c4d-content-block-heading
  >
  <c4d-content-block-copy>C4D Content Block Copy</c4d-content-block-copy>

  <c4d-button-group slot="action">
    <c4d-button-group-item href="https://example.com">
      Secondary Button
    </c4d-button-group-item>
    <c4d-button-group-item ref="https://example.com">
      Primary button
    </c4d-button-group-item>
  </c4d-button-group>

  <c4d-cta-block-item-row>
    <c4d-cta-block-item>
      <c4d-content-item-heading>Get connected</c4d-content-item-heading>
      <c4d-content-item-copy
        >IBM DevOps partners have a wide range of expertise. Find one to build
        that right solution for you.</c4d-content-item-copy
      >
      <c4d-text-cta
        slot="footer"
        cta-type="local"
        icon-placement="right"
        href="example.com"
        >Find a partner</c4d-text-cta
      >
    </c4d-cta-block-item>

    <c4d-cta-block-item>
      <c4d-content-item-heading>Learn how</c4d-content-item-heading>
      <c4d-content-item-copy
        >Dig into more self-directed larning about DevOps
        methodologies.</c4d-content-item-copy
      >
      <c4d-text-cta
        slot="footer"
        cta-type="local"
        icon-placement="right"
        href="example.com"
        >Browse tutorials</c4d-text-cta
      >
    </c4d-cta-block-item>

    <c4d-cta-block-item>
      <c4d-content-item-heading>Learn how</c4d-content-item-heading>
      <c4d-content-item-copy
        >Dig into more self-directed larning about DevOps
        methodologies.</c4d-content-item-copy
      >
      <c4d-text-cta
        slot="footer"
        cta-type="local"
        icon-placement="right"
        href="example.com"
        >Browse tutorials</c4d-text-cta
      >
    </c4d-cta-block-item>
  </c4d-cta-block-item-row>
</c4d-cta-block>
```

### With link list

```html
<c4d-cta-block>
  <c4d-content-block-heading
    >C4D Content Block Heading</c4d-content-block-heading
  >
  <c4d-content-block-copy>C4D Content Block Copy</c4d-content-block-copy>

  <c4d-button-group slot="action">
    <c4d-button-group-item target="${target}" href="https://example.com">
      Secondary Button
    </c4d-button-group-item>
    <c4d-button-group-item target="${target}" href="https://example.com">
      Primary button
    </c4d-button-group-item>
  </c4d-button-group>

  <c4d-link-list slot="link-list" type="end">
    <c4d-link-list-heading>More ways to explore DevOps</c4d-link-list-heading>
    <c4d-link-list-item href="https://example.com"> Events </c4d-link-list-item>
    <c4d-link-list-item href="https://example.com"> Blogs </c4d-link-list-item>
    <c4d-link-list-item href="https://example.com">
      Training
    </c4d-link-list-item>
    <c4d-link-list-item href="https://example.com">
      Developer resources
    </c4d-link-list-item>
    <c4d-link-list-item href="https://example.com">
      Research
    </c4d-link-list-item>
    <c4d-link-list-item href="https://example.com"> News </c4d-link-list-item>
  </c4d-link-list>
</c4d-cta-block>
```

## Props

<Props of="cds-cta-block" />
<Description markdown={markdownContents} />

## Stable selectors

See
[our README](https://github.com/carbon-design-system/carbon-for-ibm-dotcom/tree/main/packages/web-components#stable-selectors-for-analytics-and-integratione2e-testing-in-web-components)
to see how Web Components selector and `data-autoid` should be used.

| Web Components selector    | Compatibility selector              | Description |
| -------------------------- | ----------------------------------- | ----------- |
| `<c4d-cta-block>`          | `data-autoid="cds--cta-block"`      | Component   |
| `<c4d-cta-block-item>`     | `data-autoid="cds--cta-block-item"` | Component   |
| `<c4d-cta-block-item-row>` | `data-autoid="cds--cta-block-row"`  | Component   |

<Description markdown={contributing} />
