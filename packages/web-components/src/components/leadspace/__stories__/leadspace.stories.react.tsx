...
const Default = args => {
  const { alt, buttons, copy, defaultSrc, hasImage, navElements, size, title, type } = args?.Leadspace ?? {};
  return (
...
export const Short = args => {
  const { alt, defaultSrc, hasImage, navElements, size, title, type } = args?.Leadspace ?? {};
  return (
    <DDSLeadspace size={size} type={type} {...(hasImage ? { alt } : {})}>
      {navElements === navigationOptions[0] ? navigationWithTagGroup : ``}
      {navElements === navigationOptions[1] ? navigationWithBreadcrumbs : ``}
