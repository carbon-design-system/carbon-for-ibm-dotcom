import { ArgTypes, Description, Markdown } from '@storybook/addon-docs/blocks';
import markdownContents from '../../../../docs/markdown-contents.md?raw';
import contributing from '../../../../../../docs/contributing-license.md?raw';
import { cdnJs, cdnCss } from '../../../globals/internal/storybook-cdn';

# Callout with Media

> 💡 Check our
> [Stackblitz](https://stackblitz.com/github/carbon-design-system/carbon-for-ibm-dotcom/tree/main/packages/web-components/examples/stackblitz/components/callout-with-media)
> example implementation.

[![Edit @carbon/ibmdotcom-web-components](https://developer.stackblitz.com/img/open_in_stackblitz.svg)](https://stackblitz.com/github/carbon-design-system/carbon-for-ibm-dotcom/tree/main/packages/web-components/examples/stackblitz/components/callout-with-media)

**Note:** as of release v1.18.0 the copy in `C4DCalloutWithMedia` has been
updated to use `C4DCalloutWithMediaCopy` and no longer using
`C4DContentBlockCopy`

## Getting started

Here's a quick example to get you started.

## Callout with Media (Image)

### JS (via import)

```javascript
import '@carbon/ibmdotcom-web-components/es/components/callout-with-media/index.js';
import '@carbon/ibmdotcom-web-components/es/components/callout-with-media/callout-with-media-image.js';
```

<Markdown>{`${cdnJs({components: ['callout-with-media', 'callout-with-media-image']})}`}</Markdown>

<Markdown>{`${cdnCss()}`}</Markdown>

### SCSS

Callout quote has specific tokens per theme that need to emitted in the styles.
You can do this for example by adding the following:

```css
@use '@carbon/ibmdotcom-styles//scss/globals/tokens/color-tokens' as
  color-tokens;
@include add-component-tokens(color-tokens.$color-tokens);
```

### HTML

```html
<c4d-callout-with-media>
  <c4d-content-block-heading
    >lorum ipsum dolor sit ameet</c4d-content-block-heading
  >
  <c4d-callout-with-media-copy size="sm"
    >lorum ipsum dolor sit ameet</c4d-callout-with-media-copy
  >
  <c4d-callout-with-media-image
    alt="Image alt text"
    default-src="http://fpoimg.com/672x378?text=16:9&bg_color=ee5396&text_color=161616"
    heading="Lorem ipsum dolor sit amet, consectetur adipiscing elit.">
  </c4d-callout-with-media-image>
</c4d-callout-with-media>
```

## Callout with Media (Video)

### JS (via import)

```javascript
import '@carbon/ibmdotcom-web-components/es/components/callout-with-media/index.js';
import '@carbon/ibmdotcom-web-components/es/components/callout-with-media/callout-with-media-video.js';
```

<Markdown>{`${cdnJs({components: ['callout-with-media', 'callout-with-media-video']})}`}</Markdown>

<Markdown>{`${cdnCss()}`}</Markdown>

### HTML

```html
<c4d-callout-with-media>
  <c4d-content-block-heading
    >lorum ipsum dolor sit ameet</c4d-content-block-heading
  >
  <c4d-callout-with-media-copy size="sm"
    >lorum ipsum dolor sit ameet</c4d-callout-with-media-copy
  >
  <c4d-callout-with-media-video
    alt="Image alt text"
    default-src="http://fpoimg.com/672x378?text=16:9&bg_color=ee5396&text_color=161616"
    heading="Lorem ipsum dolor sit amet, consectetur adipiscing elit.">
  </c4d-callout-with-media-video>
</c4d-callout-with-media>
```

## Props

<ArgTypes of="c4d-content-block-simple" />

### `<c4d-callout-with-media-image>`

<ArgTypes of="c4d-callout-with-media-image" />

### `<c4d-callout-with-media-video>`

<ArgTypes of="c4d-callout-with-media-video" />
<Markdown>{markdownContents}</Markdown>

## Stable selectors

See
[our README](https://github.com/carbon-design-system/carbon-for-ibm-dotcom/tree/main/packages/web-components#stable-selectors-for-analytics-and-integratione2e-testing-in-web-components)
to see how Web Components selector and `data-autoid` should be used.

| Web Components selector          | Compatibility selector                        | Description |
| -------------------------------- | --------------------------------------------- | ----------- |
| `<c4d-callout-with-media>`       | `data-autoid="c4d--callout-with-media"`       | Component   |
| `<c4d-callout-with-media-copy>`  | `data-autoid="c4d--callout-with-media-copy"`  | Component   |
| `<c4d-callout-with-media-image>` | `data-autoid="c4d--callout-with-media-image"` | Component   |
| `<c4d-callout-with-media-video>` | `data-autoid="c4d--callout-with-media-video"` | Component   |

<Markdown>{contributing}</Markdown>
