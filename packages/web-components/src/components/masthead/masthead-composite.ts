...
class DDSMastheadComposite extends HostListenerMixin(LitElement) {
   ...
   *
   * TODO: remove all following once new L1 is in place
   */
   ...
   * @returns A template fragment representing a nav item.
   */
  protected _legacyRenderNavItem(link, i, autoid): TemplateResult {
    const { selectedMenuItem, currentUrlPath } = this;
    const { menuSections = [], title, titleEnglish, url, hasMegapanel } = link;
    const hasChildLink = this._legacyChildLinkChecker();
    let selected;
    ...
    if (hasMegapanel) {
      if (menuSections) {
        this.megamenuSet[i] = this._legacyRenderMegaMenu(menuSections, i);
      }
    ...
    return html`
      <dds-top-nav-menu
        ...
        ${menuSections
          ...
          .reduce(
            (acc: typeof menuItems, { menuItems }) => acc.concat(menuItems),
            []
          )
          .map(
   ...
   */
  // eslint-disable-next-line
  protected _legacyRenderMegaMenu(sections, _parentKey) {
    const { viewAllLink, highlightedItems, menu } =
      this._legacyGetHighlightedMenuItems(sections);
   ...
   */
  // eslint-disable-next-line class-methods-use-this
  protected _legacyChildLinkChecker() {
    const { currentUrlPath } = this;
    let matchFound = false;
   ...
   */
  // eslint-disable-next-line class-methods-use-this
  protected _legacyGetHighlightedMenuItems(sections) {
