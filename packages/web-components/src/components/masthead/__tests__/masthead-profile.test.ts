...
describe('dds-masthead-profile', function() {
  describe('Misc attributes', function() {
    it('should render with minimum attributes', async function() {
      ...
      expect(document.body.querySelector('dds-masthead-profile')).toMatchSnapshot({ mode: 'shadow' });
    });

    it('should render with various attributes', async function() {
      render(template({ expanded: true, menuLabel: 'menu-label-foo' }), document.body);
      await Promise.resolve();
      expect(document.body.querySelector('dds-masthead-profile')).toMatchSnapshot({ mode: 'shadow' });
    });
  });
  ...
  describe('Showing authenticated state', function() {
    it('should show the default icon in unauthenticated state', async function() {
      ...
      await Promise.resolve();
      const mastheadProfile = document.body.querySelector('dds-masthead-profile') as DDSMastheadProfile;
      const icon = mastheadProfile.shadowRoot!.querySelector('.bx--header__menu-title svg');
  ...
  describe('Toggling', function() {
    it('should toggle the menu by clicking on the trigger button', async function() {
      ...
      await Promise.resolve();
      const mastheadProfile = document.body.querySelector('dds-masthead-profile') as DDSMastheadProfile;
      const trigger = mastheadProfile.shadowRoot!.querySelector('.bx--header__menu-title') as HTMLElement;
      trigger.click();
      await Promise.resolve();
    ...
    it('should close the menu by blur', async function() {
      ...
      const trigger = mastheadProfile.shadowRoot!.querySelector('.bx--header__menu-title') as HTMLElement;
      spyOn(trigger, 'focus');
      mastheadProfile.dispatchEvent(new CustomEvent('focusout', { bubbles: true }));
      expect(mastheadProfile.expanded).toBe(false);
