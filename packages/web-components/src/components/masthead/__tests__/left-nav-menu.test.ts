...
const template = (props?) => {
  ...
  return html`
    <dds-left-nav-menu back-button-text="${ifNonNull(backButtonText)}" ?expanded="${expanded}" title="${ifNonNull(title)}">
    </dds-left-nav-menu>
  `;
};
...
describe('dds-left-nav-menu', function() {
  describe('Misc attributes', function() {
    ...
    it('should render with various attributes', async function() {
      render(template({ backButtonText: 'back-button-text-foo', expanded: true, title: 'title-foo' }), document.body);
      await Promise.resolve();
      const leftNavMenu = document.body.querySelector('dds-left-nav-menu');
      expect(leftNavMenu).toMatchSnapshot({ mode: 'shadow' });
    });
