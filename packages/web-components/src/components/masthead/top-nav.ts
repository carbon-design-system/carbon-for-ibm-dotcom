...
import { html, css, property, state, query, customElement } from 'lit-element';
...
class DDSTopNav extends StableSelectorMixin(HostListenerMixin(BXHeaderNav)) {
  ...
  static styles = [
    styles, // `styles` here is a `CSSResult` generated by custom WebPack loader.

    // styles[1] will be the sheet we alter programatically to comply with CSP rules.
    css`
      .bx--header__nav-content {
        left: 0;
      }

      :where([dir='rtl']) .bx--header__nav-content {
        right: 0;
      }
    `,
  ];
