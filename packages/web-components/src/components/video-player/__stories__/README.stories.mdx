...
```html
<cds-video-player-container id="my-video" video-id="0_ibuqxqbe">
...
> 💡 We don't test `<cds-video-player>` against arbitrary usage of Kaltura API.
...
<cds-video-player-container
  video-id="0_ibuqxqbe"
  aspect-ratio="16x9"
  playing-mode="lightbox"></cds-video-player-container>
<cds-lightbox-video-player-container></cds-lightbox-video-player-container>
```
...
`<cds-video-player>` works as the video player in lightbox media viewer.

<Props of="cds-video-player" />
...
| ----------------------- | ---------------------- | ----------- |
| `<cds-video-player>`    | N/A                    | Component   |
