 ...
 * Card footer.
 *
 * @element dds-card-hc-footer
 */
@customElement(`${ddsPrefix}-card-hc-footer`)
class DDSCardFooter extends DDSLinkWithIcon {
  /**
   * The non-link container node, used when the link of parent `<dds-card-hc>` should be used.
   */
  ...
  /**
   * `true` if the link of parent `<dds-card-hc>` should be used.
   */
  ...
  /**
   * The `href` in parent `<dds-card-hc>`.
   * `<dds-card-hc>` sets this automatically.
   */
  @property({ attribute: 'parent-href', reflect: true })
  parentHref?: string;

  /**
   * The slot in parent `<dds-card-hc>`.
   */
