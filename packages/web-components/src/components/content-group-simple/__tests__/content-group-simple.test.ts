...
const template = (props?) => {
  ...
  return html`
    <c4d-content-group-simple> ${children} </c4d-content-group-simple>
  `;
};

describe('c4d-content-group-simple', function () {
  describe('Misc attributes', function () {
    it('should render with minimum attributes', async function () {
      render(template(), document.body);
      await Promise.resolve();
      expect(
        document.body.querySelector('c4d-content-group-simple')
    ...
    it('should render with various attributes', async function () {
      ...
      await Promise.resolve(); // The update cycle of `<c4d-content-group-simple>`
