...
describe('dds-lightbox-video-player-composite', function() {
  it('should send props to video player', async function() {
    ...
    render(template({ formatCaption, formatDuration, videoId: 'video-id-foo' }), document.body);
    ...
    const videoPlayer = (videoPlayerComposite.modalRenderRoot as Element).querySelector(
  ...
  it('should stop the current video as the modal is closed', async function() {
    ...
    const modalRenderRoot = (lightboxVideoPlayerComposite as DDSLightboxVideoPlayerComposite).modalRenderRoot as Element;
    ...
    `;
    const embeddedVideoFoo = modalRenderRoot.querySelector('[data-video-id="video-id-foo"]');
    const embeddedVideoBar = modalRenderRoot.querySelector('[data-video-id="video-id-bar"]');
    const embeddedVideoBaz = modalRenderRoot.querySelector('[data-video-id="video-id-baz"]');
