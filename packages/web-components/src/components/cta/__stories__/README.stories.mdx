...
CTA components fire `dds-cta-run-action` custom event when user clicks on a CTA with `type="video"`.
...
There is `<dds-cta-container>` example component for getting started quicker. With it, you can just do:
