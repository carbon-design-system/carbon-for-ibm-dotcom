...
## Image Assets
Note that the `<FeatureCardBlockLarge />` uses custom breakpoints depending if `copy` prop has been passed in. If there is a copy, please pass a 1:1 ratio image
for the 991px breakpoint. If there is no copy, please pass in a 1:1 ratio image for the 751px breakpoint. For an example of this, take a look at `FeatureCardBlockLarge`'s image prop
in the story.
