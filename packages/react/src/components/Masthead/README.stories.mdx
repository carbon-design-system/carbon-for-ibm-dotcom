...
> 💡 Check our
> [CodeSandbox](https://githubbox.com/carbon-design-system/ibm-dotcom-library/tree/master/packages/react/examples/codesandbox/components/MastheadL1)
> example implementation.

[![Edit @carbon/ibmdotcom-web-components](https://codesandbox.io/static/img/play-codesandbox.svg)](https://githubbox.com/carbon-design-system/ibm-dotcom-library/tree/master/packages/react/examples/codesandbox/components/MastheadL1)

## Custom Events

The masthead (and side nav) currently emits two events: `megaMenuOpen` and
`megaMenuClose`. These can be useful for analytics purposes.

```javascript
document.addEventListener('megaMenuOpen', e => {
  console.log(`${e.target.id} opened megamenu`);
  // do something here
});
document.addEventListener('megaMenuClose', e => {
  console.log(`${e.target.id} closed megamenu`);
  // do something here
});
```
