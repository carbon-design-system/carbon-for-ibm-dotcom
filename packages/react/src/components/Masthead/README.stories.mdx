...
Search will always be open, it cannot be closed.
Clicking the close button will clear and focus the search input only.
> This should only be used when there are no navigation items in the masthead.

```javascript
<Masthead navigation="default" searchAlwaysOpen />;
```
