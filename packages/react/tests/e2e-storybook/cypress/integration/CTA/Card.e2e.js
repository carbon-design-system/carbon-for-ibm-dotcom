...
const _path = {
  ...
  local: 'iframe.html?id=components-cta--card&knob-type_CTA=local',
  jump: 'iframe.html?id=components-cta--card&knob-type_CTA=jump',
  external: 'iframe.html?id=components-cta--card&knob-type_CTA=external',
  download: 'iframe.html?id=components-cta--card&knob-type_CTA=download',
  video: 'iframe.html?id=components-cta--card&knob-type_CTA=video',
};
 ...
 * @param {document} root - The document object in which to search
 * @returns {string} lowercase tagname of element
 */
  ...
  checkBlockLink: () => {
    cy.visit(`/${_path.default}`)
      ...
      .then(card => {
        ...
        const root = card[0].ownerDocument;
