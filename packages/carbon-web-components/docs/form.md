...
Though form elements in `carbon-web-components` (e.g. `<cds-text-input>`) are not native form elements like `<input>`, form elements in `carbon-web-components` have some extra APIs that align well to web/framework standards that allow those form elements to participate in form.
...
button.addEventListener('click', () => {
  ...
  // Now `formData` is populated with the data in `<cds-text-input>`, etc. in the `<form>`.
